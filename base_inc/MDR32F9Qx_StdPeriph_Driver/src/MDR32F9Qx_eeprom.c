...
__RAMFUNC void EEPROM_ErasePage(uint32_t Address, uint32_t BankSelector)
{
  ...
  {
    MDR_EEPROM->ADR = (Address & 0xfffff000) + (Offset << 2);             /* Page Address */
    MDR_EEPROM->DI = 0;
...
__RAMFUNC void EEPROM_ProgramByte(uint32_t Address, uint32_t BankSelector, uint32_t Data)
{
  ...
  Data = Data << Shift;
  Mask = 0xFF << Shift;
  Tmp = EEPROM_ReadWord(((Address >> 2) << 2), BankSelector);
  EEPROM_ProgramWord(((Address >> 2) << 2), BankSelector, (Tmp & ~Mask) | (Data & Mask));
}
